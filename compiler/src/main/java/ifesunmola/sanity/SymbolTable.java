...
import ifesunmola.earth.EarthUtils;
...
enum SymbolTable {
	...
	SymbolTable() {
		...
		Stream.of(EarthType.Base.values())
			.forEach(t -> addSymbol(t.type, 0, Kind.Builtin, t));

		addSymbol("true", 0, Kind.Builtin, BoolType);
		addSymbol("false", 0, Kind.Builtin, BoolType);

		addSymbol("yap", 0, Kind.FnDecl, new FuncType(List.of(StrType), NadaType));
		addSymbol("yapln", 0, Kind.FnDecl, new FuncType(List.of(StrType),
			NadaType));

		addSymbol("intToStr", 0, Kind.FnDecl, new FuncType(List.of(IntType),
			StrType));
		addSymbol("floatToStr", 0, Kind.FnDecl, new FuncType(List.of(FloatType),
			StrType));
		addSymbol("boolToStr", 0, Kind.FnDecl, new FuncType(List.of(BoolType),
			StrType));
	}
